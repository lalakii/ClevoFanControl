...
namespace ClevoFanControl
{
    public partial class frmMain : Form
    {
        ...
        private int cpuFanRampIntervals = 5;
        private int gpuFanRampIntervals = 5;

        private IFanControl fan;
        //private static Computer computer;

        private int prevFanCPUPercentage = -1;
        private int prevFanGPUPercentage = -1;

        private int lastWLeft;
        private int lastWTop;
        ...
        private int currentCpuTemp;
        private int currentGpuTemp;
        private int prevCpuTemp;
        private int prevGpuTemp;
        private int currentCpuFan;
        private int currentGpuFan;

        private int maxCpuTemp = 0;
        private int maxGpuTemp = 0;

        private int cpuSameTempTicks = 0;
        private int gpuSameTempTicks = 0;

        private int cpuSafetyTemp = 90;
        private int gpuSafetyTemp = 85;

        private FanTable defaultCpuFanTable;
        private FanTable defaultGpuFanTable;

        private FanTable maxFanTable;
        private FanTable halfFanTable;

        private FanTable userCpuFanTable;
        private FanTable userGpuFanTable;

        private FanTable cpuFanTable;
        private FanTable gpuFanTable;

        private bool highCpuDelayFinished = false;

        private int gpuBattTicks = 0;
        private bool gpuBattMonitor = false;
        ...
        public frmMain()
        {
            ...
            pnlFanDuties.Controls[2].Left += 5;
            pnlFanDuties.Controls[4].Left += 5;
            Resize += (_, __) =>
            {
        ...
        private void SetControlText(Control.ControlCollection controls)
        {
            foreach (Control control in controls)
            {
                var btnSize = control.Font.Size;
                ...
                control.Font = new Font("Segoe UI", btnSize);
                if (control.Controls != null)
                {
                    SetControlText(control.Controls);
                }
            }
        }
        ...
        private void MnuAutoStart_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (RegistryKey key = Registry.LocalMachine.OpenSubKey(registryKey, true))
                {
                    ...
                    {
                        key.SetValue(keyName, typeof(frmMain).Assembly.Location);
                    }
                    else
                    {
        ...
        private void DelayFanInit(Action action)
        {
            Task.Run(async () =>
            {
        ...
        private void LoadFanTableAndConfig()
        {
            ...
            try
            {
                using (var sw = new StreamReader(configPath))
                {
                    mnuHide.Checked = Convert.ToBoolean(sw.ReadLine());
                    var profile = sw.ReadLine();//1
                    if (profile == "1")
                    {
                        btnProfileManual.Checked = true;
                        DelayFanInit(new Action(() => btnProfileManual_CheckedChanged(null, null)));
                    ...
                    else if (profile == "2")
                    {
                        ...
                        DelayFanInit(new Action(() => btnProfileDefault_CheckedChanged(null, null)));
                    ...
                    else if (profile == "3")
                    {
                        ...
                        DelayFanInit(new Action(() => btnProfileMax_CheckedChanged(null, null)));
                        mnuProfileManual.Checked = false;
                        mnuProfileDefault.Checked = false;
                        mnuProfile50.Checked = false;
                    ...
                    else if (profile == "4")
                    {
                        ...
                        DelayFanInit(new Action(() => btnProfile50_CheckedChanged(null, null)));
                        mnuProfileManual.Checked = false;
                        mnuProfileDefault.Checked = false;
                        mnuProfileMax.Checked = false;
                    ...
                    wLeft = Convert.ToInt32(sw.ReadLine());//2
                    wTop = Convert.ToInt32(sw.ReadLine());//3

                    lastWLeft = wLeft;
                    lastWTop = wTop;

                    btnAlwaysOnTop.Checked = Convert.ToBoolean(sw.ReadLine());//4
                    btnACFans.Checked = Convert.ToBoolean(sw.ReadLine());//5
                    txtCpuSafetyTemp.Value = Convert.ToInt32(sw.ReadLine());//6
                    cpuSafetyTemp = Convert.ToInt32(txtCpuSafetyTemp.Value);
                    txtGpuSafetyTemp.Value = Convert.ToInt32(sw.ReadLine());//7
                    gpuSafetyTemp = Convert.ToInt32(txtGpuSafetyTemp.Value);
                    btnGpuBattMonitor.Checked = Convert.ToBoolean(sw.ReadLine());//8
        ...
        private readonly string configPath = Path.Combine(Path.GetDirectoryName(typeof(frmMain).Assembly.Location), typeof(frmMain).Assembly.GetName().Name + ".ini");

        private void MnuHide_Click(object sender, System.EventArgs e)
        {
            mnuHide.Checked = !mnuHide.Checked;
            SaveFanTableAndConfig();
        }
        ...
        private void MnuAbout_Click(object sender, System.EventArgs e)
        {
            try
            {
                Process.Start("https://github.com/lalakii/ClevoFanControl");
            }
            catch { }
        }
